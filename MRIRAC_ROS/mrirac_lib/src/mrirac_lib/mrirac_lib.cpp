...
bool RobotMovements::Fr3_GripperAction(const double width, actionlib::SimpleActionClient<franka_gripper::MoveAction> *fr3_finger_client)
{
    ...
    fr3_finger_client->sendGoal(goal);

    if (fr3_finger_client->waitForResult(ros::Duration(5.0)))
    {
        fr3_finger_client->getResult();
        return true;
    }
    else
    {
        fr3_finger_client->cancelAllGoals();
...
bool RobotMovements::Fr3_GripperGraspAction(const double force, actionlib::SimpleActionClient<franka_gripper::GraspAction> *fr3_finger_client)
{
    ...
    {
        fr3_finger_client->cancelAllGoals();
        ROS_WARN_STREAM("The gripper grasp action timed-out");
        return false;
    }
}
